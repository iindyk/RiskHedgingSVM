...
def graph_covariate_shift(n):
    ...
    # plot original data and concept drift
    plt.subplot(122)
    scatter = plt.scatter([float(i[0]) for i in data], [float(i[1]) for i in data], c=colors, cmap=plt.cm.coolwarm)
    plt.legend(handles=scatter.legend_elements()[0], labels=_legend)
    x = np.arange(-0.5, 1.5, 0.01)
    ...
    plt.scatter([float(i[0]) for i in data if i[1]<3*i[0]-0.85],
                ...
                label=legend, cmap=plt.cm.coolwarm)
    plt.plot(x, np.square(x-0.5)*3+0.25, 'g-', linewidth=3, label='true decision boundary')
    plt.xlim((0, 1))
    plt.ylim((0, 1))
    plt.tick_params(axis='x', which='both', bottom=False, labelbottom=False)
