...
for j in range(3):
    for i in range(n_pois):
        ...
        target_sets[j].append(functions.get_histogram(pois_x, a, b_, n_bins))
    ...
    for i in range(n_pois):
        ...
        target_sets[j].append(functions.get_histogram(cov_shift_x, a, b_, n_bins))
        target_labels[j].append(1)

    # generate concept drifts
    for i in range(n_pois):
        indices = np.random.randint(0, n, n_pois)
        conc_drift_data = data.get_random_concept_drift(data[indices], labels[indices], dist)
        conc_drift_labels = labels[indices]
        conc_drift_x = np.array([conc_drift_labels[i] *
                                (np.dot(conc_drift_data[i], w)+b)/np.linalg.norm(w) for i in range(n_pois)])
        target_sets[j].append(functions.get_histogram(conc_drift_x, a, b_, n_bins))
        target_labels[j].append(1)
    if j == 0:
